/**
 * ROTTA DO AÇAÍ - PAINEL DE GESTÃO DA LOJA
 * Controle de pedidos ao vivo (Kanban), comanda térmica, estoque rápido e fechamento de caixa.
 */
...
// ==========================================================================
// 2. STATUS DA LOJA (ABERTA / FECHADA)
// ==========================================================================
...
function renderKanbanBoard() {
  ...
  const newBadge = document.getElementById('kanban-new-badge');
  if (counts.novo > 0) {
    newBadge.textContent = counts.novo;
...
function createOrderCardElement(order) {
  ...
  const customerHtml = `
    <div class="space-y-0.5 text-xs">
      <div class="flex items-center justify-between">
        ...
        <a href="https://api.whatsapp.com/send?phone=55${order.customer.phone.replace(/\D/g, '')}" target="_blank" class="text-[11px] text-emerald-600 hover:underline font-bold flex items-center gap-1">
          <span>💬 WhatsApp</span>
        </a>
      </div>
      ${order.address ? `
        <p class="text-[11px] text-gray-500 line-clamp-2">
          ...
          ${order.address.ref ? `(${order.address.ref})` : ''}
        </p>
      ` : '<p class="text-[11px] text-blue-600">Cliente irá retirar no balcão da loja</p>'}
    </div>
  `;
        ...
        <div class="border-b border-gray-200/50 pb-1 last:border-0 last:pb-0">
          ...
          ${item.freeToppings && item.freeToppings.length > 0 ? `
            <div class="text-[9px] text-gray-600">
              ✓ ${item.freeToppings.map(t => t.name).join(', ')}
            </div>
          ` : ''}

          ${item.paidAddons && item.paidAddons.length > 0 ? `
            <div class="text-[9px] text-amber-700 font-semibold">
              ★ ${item.paidAddons.map(a => a.name).join(', ')}
            </div>
          ` : ''}

          ${item.notes ? `
            <div class="text-[9px] italic text-purple-600 bg-purple-50 p-0.5 rounded mt-0.5">
              Obs: "${item.notes}"
            </div>
          ` : ''}
        </div>
  ...
  if (order.status === 'novo') {
    actionButtonsHtml = `
      <div class="grid grid-cols-2 gap-2 pt-1">
        <button onclick="openReceiptModal('${order.id}')" class="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-2 rounded-lg transition flex items-center justify-center space-x-1">
          <span>🖨️</span>
          <span>Comanda</span>
        </button>
        <button onclick="advanceOrderStatus('${order.id}', 'preparo')" class="text-xs bg-amber-500 hover:bg-amber-600 text-white font-black py-2 px-2 rounded-lg shadow transition flex items-center justify-center space-x-1">
          <span>🥣</span>
          <span>Aceitar</span>
        </button>
      </div>
    ...
    actionButtonsHtml = `
      <div class="grid grid-cols-2 gap-2 pt-1">
        ...
        <button onclick="advanceOrderStatus('${order.id}', 'entrega')" class="text-xs bg-purple-600 hover:bg-purple-700 text-white font-black py-2 px-2 rounded-lg shadow transition flex items-center justify-center space-x-1">
          <span>🛵</span>
          <span>Despachar</span>
        </button>
      </div>
    ...
    actionButtonsHtml = `
      <div class="grid grid-cols-2 gap-2 pt-1">
        ...
        <button onclick="advanceOrderStatus('${order.id}', 'concluido')" class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white font-black py-2 px-2 rounded-lg shadow transition flex items-center justify-center space-x-1">
          <span>✅</span>
          <span>Concluir</span>
        </button>
      </div>
    ...
    actionButtonsHtml = `
      <div class="pt-1">
        <button onclick="openReceiptModal('${order.id}')" class="w-full text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-1.5 rounded-lg transition">
          🖨️ Reemitir Comanda
        </button>
      </div>
    `;
  ...
  card.innerHTML = headerHtml + customerHtml + itemsHtml + paymentHtml + actionButtonsHtml;
  return card;
}
...
function confirmClearOrders() {
  if (confirm('Deseja realmente limpar os pedidos concluídos do histórico?')) {
    const orders = window.Store.getOrders().filter(o => o.status !== 'concluido');
    localStorage.setItem('rotta_orders', JSON.stringify(orders));
    window.Store.broadcast('ORDERS_UPDATED', orders);
    renderKanbanBoard();
    renderFinancialMetrics();
  }
}
...
// ==========================================================================
// 5. COMANDA DE MONTAGEM & IMPRESSÃO TÉRMICA
// ==========================================================================
function openReceiptModal(orderId) {
  ...
  container.innerHTML = `
    <div class="text-center pb-2 border-b border-dashed border-gray-400">
      ...
      <div class="text-[10px] text-gray-600">WhatsApp: ${config.phone}</div>
    ...
    <div class="py-2 border-b border-dashed border-gray-400 space-y-0.5 text-[11px]">
      ...
      ${order.address ? `
        <div class="mt-1 bg-gray-50 p-1 rounded">
          <strong>ENDEREÇO:</strong><br>
          ${order.address.street}, ${order.address.number} - ${order.address.neighborhood}<br>
          ${order.address.ref ? `Ponto de Ref: ${order.address.ref}` : ''}
        </div>
      ` : ''}
    </div>
    ...
    <div class="py-2 border-b border-dashed border-gray-400 space-y-2">
      ...
      ${order.items.map(item => `
        <div class="text-[11px] pb-1 border-b border-gray-100 last:border-0">
          ...
          ${item.base ? `<div class="font-semibold text-gray-800 ml-2">» Base: ${item.base}</div>` : ''}

          ${item.freeToppings && item.freeToppings.length > 0 ? `
            <div class="text-gray-700 ml-2">
              » Grátis: ${item.freeToppings.map(t => t.name).join(' + ')}
            </div>
          ` : ''}

          ${item.paidAddons && item.paidAddons.length > 0 ? `
            <div class="font-bold text-gray-900 ml-2">
              ★ Extras: ${item.paidAddons.map(a => a.name).join(' + ')}
            </div>
          ` : ''}

          ${item.notes ? `
            <div class="italic text-gray-800 ml-2 bg-yellow-50 p-0.5">
              OBS: "${item.notes}"
            </div>
          ` : ''}
        </div>
    ...
    <div class="pt-2 text-[11px] space-y-1">
      <div class="flex justify-between">
        <span>Subtotal:</span>
        <span>${window.Store.formatCurrency(order.subtotal)}</span>
      </div>
      ${order.deliveryFee > 0 ? `
        <div class="flex justify-between">
          <span>Taxa de Entrega:</span>
          <span>${window.Store.formatCurrency(order.deliveryFee)}</span>
        </div>
      ` : ''}
      <div class="flex justify-between font-black text-sm pt-1 border-t border-gray-400">
        <span>VALOR TOTAL:</span>
        <span>${window.Store.formatCurrency(order.total)}</span>
      </div>
    ...
    <div class="p-3 bg-amber-50/50 rounded-xl border border-amber-200 flex items-center justify-between">
      <div class="flex items-center space-x-2.5">
        ...
        <div>
          ...
          <span class="text-[11px] text-amber-700 font-semibold">+ ${window.Store.formatCurrency(addon.price)}</span>
        </div>
      </div>
      <label class="relative inline-flex items-center cursor-pointer">
...
function renderFinancialMetrics() {
  ...
  const cashOrders = validOrders.filter(o => o.paymentMethod === 'dinheiro');

  document.getElementById('metric-pix-total').textContent = window.Store.formatCurrency(pixOrders.reduce((s, o) => s + o.total, 0));
  document.getElementById('metric-pix-count').textContent = `${pixOrders.length} ped`;

  document.getElementById('metric-card-total').textContent = window.Store.formatCurrency(cardOrders.reduce((s, o) => s + o.total, 0));
  document.getElementById('metric-card-count').textContent = `${cardOrders.length} ped`;

  document.getElementById('metric-cash-total').textContent = window.Store.formatCurrency(cashOrders.reduce((s, o) => s + o.total, 0));
  document.getElementById('metric-cash-count').textContent = `${cashOrders.length} ped`;
}
...
function saveStoreSettings(e) {
  ...
  window.Store.saveConfig(config);
  alert('Configurações da Rotta do Açaí salvas com sucesso!');
}

// ==========================================================================
// 9. SINCRONIZAÇÃO EM TEMPO REAL & ALERTA SONORO
// ==========================================================================
function setupSyncListener() {
  window.Store.onSync(data => {
    if (data.type === 'NEW_ORDER') {
      ...
      renderKanbanBoard();
      renderFinancialMetrics();
    } else if (data.type === 'ORDER_STATUS_CHANGED' || data.type === 'STORAGE_EVENT') {
      renderKanbanBoard();
      renderFinancialMetrics();
    }
  });
